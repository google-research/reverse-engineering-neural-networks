...
Adapted from flax/examples/lm1b/model.py.
"""
...
class TransformerLM(nn.Module):
  """Transformer model for language modeling and classification."""
  ...
  def apply(self,
    ...
    Input sequences will first be encoded, then used for classification.
    ...
    Args:
      ...
      attention_dropout_rate: dropout rate for attention weights
      cache: flax autoregressive cache for fast decoding.
      causal_mask: Use causal_mask or not.
    ...
    x = jnp.multiply(x, padding_mask)
    # Get input sequence encoding.
    x = jnp.sum(x, axis=1) / jnp.sum(padding_mask, axis=1)
    # Two classes for sentiment analysis.
    logits = nn.Dense(x,
                      output_classes,
                      kernel_init=nn.initializers.xavier_uniform(),
                      bias_init=nn.initializers.normal(stddev=1e-6))
